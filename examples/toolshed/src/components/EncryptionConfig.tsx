...
function EncryptionConfig({ state }: consumeProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      ...
      <div>
        <input value={encryptionContent.rawMessage} size={55} onChange={handleChange} name="rawMessage" />

        <button onClick={async () => await cryptMessage()}>Encrypt</button>
      </div>
      <div>
        <input
          value={encryptionContent.encryptedMessage.toString('hex')}
          size={55}
      ...
      {encryptionContent.decryptedMessage !== '' && (
        <p>
          {' '}
          <span style={{ fontWeight: 'bold' }}>Decrypted:</span> {encryptionContent.decryptedMessage}{' '}
        </p>
      )}
    </div>
  )
